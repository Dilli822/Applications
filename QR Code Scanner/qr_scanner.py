...
def save_data_to_file(data):
    """Save the QR code data with a timestamp to a text file."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open("qr_code_data.txt", "a") as file:
        file.write(f"{timestamp}: {data}\n")
    print(f"Saved QR code data: {data} at {timestamp}")
...
def scan_qr_code():
    try:
        ...
        print("Waiting for QR code... (Press 'Esc' to quit)")
        ...
        while True:
            ...
            if bbox is not None and data:
                ...
                # Save the QR code data with a timestamp
                ...
                while time.time() - start_time < 6:
                    ...
                    # Check if 'Esc' is pressed during the idle period
