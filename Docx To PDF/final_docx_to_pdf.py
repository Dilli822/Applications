...
#     output_path = os.path.join(os.path.dirname(docx_path), f'{base_name}.pdf')
...
# # Usage
# convert_docx_to_pdf('Artificial_General_Intelligence_Concept_State_of_t.docx')
...
def get_paragraph_style(paragraph, styles):
    """Get the corresponding ReportLab style for a DOCX paragraph."""
    if paragraph.style.name.startswith('Heading'):
...
def convert_docx_to_pdf(docx_path):
    ...
    try:
        ...
        # Iterate through all elements in the DOCX file
        for paragraph in doc.paragraphs:
            # Get the appropriate style
            style = get_paragraph_style(paragraph, styles)
        ...
        for rel in doc.part.rels.values():
            if "image" in rel.target_ref:
                ...
                img = Image(temp_img_path)
                img.drawHeight = 1.5 * inch  # Adjust the height as needed
                img.drawWidth = 1.5 * inch    # Adjust the width as needed
                elements.append(img)
                elements.append(Spacer(1, 0.1 * inch))
