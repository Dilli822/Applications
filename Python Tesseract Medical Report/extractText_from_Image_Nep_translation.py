...
def check_url_validity(url):
    """Check if the provided URL is valid and reachable."""
    try:
...
if __name__ == "__main__":
    # image_url = "https://images.sampletemplates.com/wp-content/uploads/2017/02/Medical-Reports-of-Patients.png"  
    image_url = ""
    local_image_path = "Medical-Reports-of-Patients.webp"  # Local image path

    # Check if the URL is valid
    if check_url_validity(image_url):
        print("Valid URL. Downloading image...")
        image = download_image(image_url)  # Download the image
    else:
        print("Invalid URL. Using local image path...")
        image = cv2.imread(local_image_path)  # Read the local image

    extracted_text = extract_text_from_image(image)  # Extract text from the image
